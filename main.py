# -*- coding: utf-8 -*-
"""
Created on Wed Apr 28 11:46:17 2021
...
if hasattr(Qt, 'AA_EnableHighDpiScaling'):
    QW.QApplication.setAttribute(Qt.AA_EnableHighDpiScaling, True)

if hasattr(Qt, 'AA_UseHighDpiPixmaps'):
    QW.QApplication.setAttribute(Qt.AA_UseHighDpiPixmaps, True)
...
class MainWindow(QW.QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowIcon(QIcon('Icons/XML_logo.png'))
        self.statusBar()
        self.init_ui()
        self.init_actions()
        self.showMaximized()
    ...
    # Central Widget (Tab Widget)
        self._xmapstab = XMapsTab(self)
        self._minmaptab = MinMapTab(self)
        
        self.tabWidget = QW.QTabWidget()
        self.tabWidget.addTab(self._xmapstab, 'Input Maps')
        self.tabWidget.addTab(self._minmaptab, 'Classified Mineral Maps')
        # self.tabWidget.tabBar().setAcceptDrops(True)
        self.setCentralWidget(self.tabWidget)
        
        self.setStyleSheet('''QTabWidget {background-color: rgb(169, 185, 188);}'''
                           '''QTabBar {color: black;}'''
                           '''PathLabel {color: black;}''')
    ...
    # ========================   ACTIONS   ======================== #
        ...
        addXmapsAction = QW.QAction('&Input Maps', self)
        addXmapsAction.setShortcut('Ctrl+Alt+I')
        addXmapsAction.triggered.connect(lambda: self._xmapstab.loadMaps())
        
        # Import Mineral Maps Action [--> Import subMenu --> File Menu]
        addMinmapAction = QW.QAction('&Mineral Maps', self)
        addMinmapAction.setShortcut('Ctrl+Alt+M')
        addMinmapAction.triggered.connect(lambda: self._minmaptab.loadMaps())
        
        # Edit Preferences [--> File Menu]
        PreferenceAction = QW.QAction(QIcon('Icons/wrench.png'),
                                      '&Preferences', self)
        PreferenceAction.setShortcut('Ctrl+Alt+P')
        ...
        dsBuilderTool = QW.QAction(QIcon('Icons/compile_dataset.png'), 
                                   'Dataset &Builder', self)
        dsBuilderTool.setShortcut('Ctrl+Alt+B') 
        dsBuilderTool.setStatusTip('Design a new ground-truth dataset')
        dsBuilderTool.triggered.connect(self.run_DatasetBuilder)
        
        # ML Model Builder [--> Toolbar] & [--> Classification Menu] 
        modelLearnerTool = QW.QAction(QIcon('Icons/merge.png'), 
                                      'Model &Learner', self)
        ...
        classifyTool = QW.QAction(QIcon('Icons/classify.png'),
                                   'Mineral &Classifier', self)
        classifyTool.setShortcut('Ctrl+Alt+C') 
        classifyTool.setStatusTip('Predict mineral maps')
        classifyTool.triggered.connect(self.run_MineralClassifier)

        # Phase Refiner Tool [--> Toolbar] & [Post-classification Menu]
        phaseRefinerTool = QW.QAction(QIcon('Icons/refine.png'),
                                      'Phase &Refiner', self)
    ...
    # ========================   MENU   ======================== #
        ...
        importSubMenu = fileMenu.addMenu(QIcon('Icons/generic_add_black.png'),
                                         '&Import...')
        importSubMenu.addActions((addXmapsAction, addMinmapAction))
        ...
        for menu in menuBar.children():
            menu.setStyleSheet('''QMenu {background-color: rgb(249, 249, 249);}'''
                               '''QMenu::item:selected {background-color: rgb(238, 181, 182);}'''
                               '''QMenu::item:right-arrow {color: black;}''')
            
        

    # ========================   TOOLBAR   ======================== #
        toolBar = QW.QToolBar()
        toolBar.setStyleSheet('''QToolButton {background-color: rgb(138, 163, 189);}'''
                              '''QToolButton:hover {background-color: rgb(238, 181, 182);}''')
    ...
    def run_MineralClassifier(self):
        if len(_loadedXMapsData) == 0:
            QW.QMessageBox.critical(self, 'Missing Input Maps',
                                    'Please load some data in the "X-Ray Maps" Tab first.')
        else:
            self.classifyDialog = dialogs.MineralClassifier((_loadedXMapsPath, _loadedXMapsData), 
                                                            (_loadedMinMapsPath, _loadedMinMapsData),
                                                            self)
            self.classifyDialog.show()
        
    def run_DatasetBuilder(self):
        self.dsBuilderDialog = dialogs.DatasetBuilder(self)
        self.dsBuilderDialog.show()
    ...
    def run_PhaseRefiner(self):
        minMap = self._minmaptab.currentMap
        if minMap is None:
    ...
    def closeEvent(self, event):
        choice = QW.QMessageBox.question(self, "Exit confirm", "Do you really want to exit the app?",                      
...
class XMapsTab(QW.QWidget):
    
    def __init__(self, parent):
        self.parent = parent
        super(XMapsTab, self).__init__(parent=self.parent)
    ...
    # Reset zoom Action in X-Ray Maps NavTbar
        ...
        self.resetZoomAction.triggered.connect(self.XMapsView.resetZoom)
    ...
    def scaleColormap(self, enabled):
        self.XMapsView.alterScaledColorMap(enabled, _loadedXMapsData)
    ...
    # Correct number of selected maps (1 to 4)
        ...
        self.RGBACanvas.update_canvas('RGB(A) composite map', RGBAmap)
    ...
    def showMap(self, item):
        if item is not None:
            ...
            try:
                ...
                self.XMapsView.update_canvas(name, data)
                self.ROIAction.setEnabled(True)
                self.histCanvas.update_canvas(name, data, self.ROIcoords)
                self.scaler.hide()
    ...
    def scroll_maps(self, direction):  
        if not self.XMapsView.wheelZoom:
            curr_idx = self.TOC.currentRow()
            if curr_idx != -1:
                tot_items = self.TOC.count()
                if direction == 'down':
                    curr_idx = (curr_idx + 1) % tot_items
                elif direction == 'up':
                    curr_idx = (curr_idx - 1) % tot_items
                else:
                    return 
...
class MinMapTab(QW.QWidget):
    def __init__(self, parent):
        self.parent = parent
        super(MinMapTab, self).__init__(parent=self.parent)
    ...
    # Mineral Map View Area
        ...
        self.MinMapView.mpl_connect('scroll_event',  
                                   lambda evt: self.scroll_maps(evt.button))
        CF.shareAxis(self.parent._xmapstab.XMapsView.ax, self.MinMapView.ax, 
    ...
    # Reset Zoom Action in MinMap NavTbar
        ...
        self.resetZoomAction.triggered.connect(self.MinMapView.resetZoom)
    ...
    # Labelize Action in Mode BarChart NavTbar
        ...
        self.labelizeAction.toggled.connect(self.modeView.labelize)        
    ...
    def update_TOC(self):
        self.TOC.clear()
        self.TOC.addItems([CF.path2fileName(path) for path in _loadedMinMapsPath])
    ...
    def loadMaps(self, paths=None): 
        ...
        if paths:
            ...
            for n, p in enumerate(paths, start=1):
                if not progBar.wasCanceled():
                    try:
                        if p not in _loadedMinMapsPath:
                            # populate maps data list
                            _loadedMinMapsData.append(np.loadtxt(p, dtype='U8'))
                            # populate maps path list
                            _loadedMinMapsPath.append(p)
                            # populate maps color palette
                            _loadedMinMapsPalette.append(False)
    ...
    def delMaps(self): 
        ...
        if len(selected):
            ...
            if choice == QW.QMessageBox.Yes:
                for idx in sorted(selected, reverse=True):
                    del _loadedMinMapsPath[idx]
                    del _loadedMinMapsData[idx]
                    del _loadedMinMapsPalette[idx]
                self.update_TOC()
    ...
    def show_minMap(self, item):
        # Skip the update if there's only one loaded map and it is already plotted
        if self.currentMap is not None and self.TOC.count() == 1:
            return
        idx = self.TOC.currentRow()
        path = _loadedMinMapsPath[idx]
        data = _loadedMinMapsData[idx]
        colors = _loadedMinMapsPalette[idx]
        name = item.text()
        
        try:
            self.highlight_btn.setChecked(False)
            self.curr_imgPath.set_fullpath(path, predict_display=True)
            self.MinMapView.update_canvas(name, data, colors)
            self.currentMap = data
            self.legend.update()
            self.update_modePlot(self.currentMap)
        ...
        pMapPath = CF.extendFileName(path, '_probMap')
        if exists(pMapPath):
            self.show_pMap(pMapPath)
        else:
            self.pMapView.clear_canvas(data.shape)
            self.currentPMap = None
    ...
    def update_modePlot(self, mapData):
        if mapData is not None:
        # We prefer to get the unique values as IDs from the legend and then decode 
        # them to labels for efficiency purpose in terms of processing time.
            modeDict = self.legend.get_modeDict(keys='ID', sortByMode=True)
            IDs, mode = zip(*modeDict.items())
            lbl = CF.decode_labels(IDs, self.MinMapView.dataDict)
        # It requires less time to get the color dict from ID keys 
            col_dict = CF.orderDictByList(self.MinMapView.get_colorDict(keys='ID'), IDs)
            self.modeView.update_canvas('Mode', mode, lbl, colors=list(col_dict.values()))
            ...
            if ok and name != '':
                if len(name) > 8:
                    ...
                    if choice.clickedButton().text() == '&Yes':
                        ...
                        _loadedMinMapsData[idx] = self.currentMap
                        
                        # Update mineral map canvas, legend and mode canvas
                        self.MinMapView.update_canvas(mapName, self.currentMap)
                        self.legend.update()
                        self.update_modePlot(self.currentMap)
                        ...
                        if choice.checkBox().isChecked():
                            try:
                                np.savetxt(_loadedMinMapsPath[idx], 
                                           self.currentMap, fmt='%s')
                            except: 
                                QW.QMessageBox.critical(self, 'Error',
    ...
    def randomize_color(self):
        if self.currentMap is not None:
            n_phases = len(self.MinMapView.dataDict)
            randCol = self.MinMapView.get_randColors(n_phases) 
            self.MinMapView.alter_ColorMap(randCol)
            self.legend.update()
            self.update_modePlot(self.currentMap)
        
    def save_palette(self):
        if self.legend.count() > 0:
            idx = self.TOC.currentRow()
            _loadedMinMapsPalette[idx] = self.MinMapView.curr_colors
        ...
        if self.legend.count() > 0 and self.TOC.currentItem() is not None:
            if enable:
                if self.legend.currentItem() is not None:
                    ...
                    condition = self.currentMap != phase
                    self.MinMapView.mask_data(condition)
                    if self.currentPMap is not None:
                        self.pMapView.mask_data(condition)
                else: 
                    self.highlight_btn.setChecked(False)
            else:
                self.MinMapView.update_canvas(self.TOC.currentItem().text(), 
                                              self.currentMap)  
                if self.currentPMap is not None:
                    self.pMapView.update_canvas('Probability Map', self.currentPMap)
    ...
    def toggleEditMode(self, toggled):
        ...
        self.rectSel.set_active(toggled)
        self.rectSel.set_visible(toggled)
        self.MinMapView.enablePicking(toggled)
    ...
    def exportArray(self):
        ...
        if choice.clickedButton().text() == '&Yes':
            ...
            if outpath:
                ...
                self.MinMapView.exportIdData(outpath, includeDict)
                
    def scroll_maps(self, direction):  
        if not self.MinMapView.wheelZoom:
            curr_idx = self.TOC.currentRow()
            if curr_idx != -1:
                tot_items = self.TOC.count()
                if direction == 'down':
...
if __name__ == "__main__":
    ...
    loader = QW.QSplashScreen(loader_bg, 
                              flags=Qt.WindowStaysOnTopHint | Qt.FramelessWindowHint)
    loader.show()
    loader.showMessage('\nLoading app...', Qt.AlignHCenter, Qt.white)
    
    app.setStyle('Fusion')
    palette = app.palette()
    palette.setColor(QPalette.Active, QPalette.Window, QColor(25, 35, 45)) #QColor(169, 185, 188)) 
    palette.setColor(QPalette.Inactive, QPalette.Window, QColor(46, 64, 82))
    
    palette.setColor(QPalette.Active, QPalette.Base, QColor(64, 89, 115))    
    palette.setColor(QPalette.Inactive, QPalette.Base, QColor(87, 101, 115))
    palette.setColor(QPalette.Disabled, QPalette.Base, Qt.gray)
    ...
    palette.setColor(QPalette.Button, QColor(249, 249, 249)) 
    ...
    palette.setColor(QPalette.Highlight, QColor(238, 181, 182))
